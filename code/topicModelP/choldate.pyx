...
cpdef np.ndarray cholupdate(np.ndarray[FLOAT_t, ndim=2] R, np.ndarray[FLOAT_t, ndim=1] x):
    ...
    for k in range(p):
        ...
        #r = sqrt(R[<unsigned int>k, <unsigned int>k]**2 + x[<unsigned int>k]**2)
        ...
        for i in range(<unsigned int>(k+1),<unsigned int>p):
            ...
            #x[<unsigned int>i] = cabs(c * x[<unsigned int>i] - s * R[<unsigned int>k,<unsigned
            #    int>i])
            x[<unsigned int>i] = c * x[<unsigned int>i] - s * R[<unsigned int>k,<unsigned
                int>i]
    return R
